...
1.  **Sommes** - Un prénom est populaire *si le nombre total d'enfants qui le portent est important lorsque vous l'additionnez sur plusieurs années*.
2.  **Rangs** - Un prénom est populaire *s'il se classe régulièrement parmi les meilleurs prénoms d'une année à l'autre*.
...
2.  le nombre maximum d'enfants qui ont eu le prénom "Marie" en une seule année
3.  le nombre moyen d'enfants qui ont été prénommés "Marie" par an
...
Comment pouvons-nous appliquer `summarise()` pour trouver les prénoms les plus populaires dans `prenoms` ?

Vous avez vu comment calculer le nombre total d'enfants qui portent un prénom donné, ce qui fournit l'une de nos mesures de popularité (le nombre total d'enfants qui ont un prénom donné) :
...
2.  *Puis* il sera résumé dans un jeu de données regroupées par année (c'est-à-dire le résultat ci-dessus)
3.  *Puis* il sera résumé dans un jeu de données final qui n'est pas groupé.
...
Pourquoi pourrait-il y avoir une différence entre la *proportion* d'enfants qui reçoivent un prénom au fil du temps et le *nombre* d'enfants qui reçoivent le prénom ?
...
2.  **Rangs** - Un prénom est populaire *s'il se classe régulièrement parmi les prénoms les plus populaires d'une année à l'autre*.
...
2.  **Les arithmétique modulaires**: `%/%` (division entière) et `%%` (reste)
3.  **Les comparaisons logiques**, `<`, `<=`, `>`, `>=`, `!=`
4.  **Les fonctions logarithmiques**  - `log(x)`, `log2(x)`, `log10(x)`
5.  **Les offsets** - `lead (x)` et `lag (x)` (trouvent la valeur précédente ou la valeur suivante dans un vecteur)
6.  **Les agrégats cumulatifs** - `cumsum(x)`, `cumprod(x)`, `cummin(x)`, `cummax(x)`, `cummean(x)`
7.  **Les classements (rangs)** - `min_rank(x)`, `row_number(x)`, `dense_rank(x)`, `percent_rank(x)`, `cume_dist(x)`, `ntile(x)`
...
Dans l'exercice précédent, nous avons attribué des classements sur l'ensemble des données. Par exemple, à l'exception des égalités, il n'y avait qu'un seul 1er *dans l'ensemble des données*, un seul 2eme, etc. Pour calculer un score de popularité sur plusieurs années, vous devrez faire quelque chose de différent : vous devrez attribuer des classements au sein de groupes d'année et de sexe. Maintenant, il y aura un 1er *dans chaque groupe d'année et de sexe*.
...
2.  Attribuez un rang à chaque prénom en fonction des valeurs décroissantes de `prop`
3.  Regroupez les données par `name` et `sex`
4.  Calculez le classement médian pour chaque combinaison nom-sexe
5.  Organisez les résultats de sorte que les noms avec le score le plus faible apparaissent en haut du jeu de données.
...
Ensemble, ces six fonctions créent une *grammaire* pour la manipulation des données. Vous pouvez l'utiliser pour manipuler les données de manière sophistiquée, étape par étape. Ces fonctions ciblent les tâches quotidiennes de l'analyse des données. Peu importe les types de données avec lesquelles vous travaillez, vous découvrirez que :

1.  *Les jeux de données contiennent souvent plus d'informations que nécessaire*
2.  *Les jeux de données impliquent plus d'informations qu'ils n'en affichent*
