...
```{r ex7, exercise = TRUE}
#table4a %>%
#  pivot_longer(cols = c(____ ), names_to = "___", values_to = "____")
```
...
`pivot_wider()` donnera à chaque valeur unique de la colonne **`names_from`** sa propre colonne. 
Le nom de la valeur deviendra le nom de la colonne. `pivot_wider()` redistribuera ensuite les valeurs de la colonne **`values_from`** à travers les nouvelles colonnes d'une manière qui préserve chaque relation dans le jeu de données d'origine.
...
table4b %>% 
  pivot_longer(cols = -country, names_to = "year", values_to = "population")
pollution %>% 
  pivot_wider(names_from = size,  value = amount)
```
...
## Exercise: Filles et garçons dans le jeu de données prenoms
...
```{r ex13, exercise = TRUE, message=FALSE}
prenoms %>%
  group_by(year, sex) %>% 
  summarise(total = sum(n))
```
